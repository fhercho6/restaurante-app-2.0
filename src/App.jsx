// src/App.jsx - CORTESÍAS INTEGRADAS + ADMIN DASHBOARD + CORRECCIONES
...
export default function App() {
  ...
  // ESTADÍSTICAS DEL TURNO (CON CORTESÍAS)
  const [sessionStats, setSessionStats] = useState({ 
      ...
      totalExpenses: 0, 
      totalCostOfGoods: 0, 
      courtesyTotal: 0, // VALOR DE VENTA REGALADO
      courtesyCost: 0, // COSTO DE LO REGALADO
      expensesList: [],
      soldProducts: [] 
  });
  ...
  // --- LÓGICA DE AGREGACIÓN DE VENTAS (PRODUCTOS, COSTOS, GANANCIA Y CORTESÍAS) ---
  useEffect(() => { 
      ...
      const unsubSales = onSnapshot(qSales, (snap) => { 
          ...
          let courtesyTotalVal = 0; // Valor de venta de lo regalado
          let courtesyCostVal = 0; // Costo de lo regalado
          let totalCostCalc = 0; // Costo de lo vendido REAL (sin cortesías)
          ...
          snap.forEach(doc => { 
              ...
              // 1. Dinero (Solo si no es cortesía)
              if (!isCourtesy) {
                  if (sale.payments && Array.isArray(sale.payments)) { 
                      ...
                      if (method.includes('efectivo')) { const change = parseFloat(sale.changeGiven) || 0; const received = parseFloat(sale.amountReceived) || total; if(sale.amountReceived) cash += (received - change); else cash += total; } else if (method.includes('qr')) qr += total; else if (method.includes('tarjeta')) card += total; 
                  }
              }
              ...
              if (sale.items && Array.isArray(sale.items)) {
                  sale.items.forEach(item => {
                      ...
                      // Si es cortesía, sumamos a los contadores de cortesía
                      if (isCourtesy) {
                          courtesyTotalVal += (price * qty);
                          courtesyCostVal += (cost * qty);
                      } else {
                          // Si es venta real, sumamos al costo de ventas
                          totalCostCalc += (cost * qty);
                      }

                      // Agrupación para inventario (Se cuenta todo, vendido o regalado, porque salió del stock)
                      if (!productMap[key]) {
                          productMap[key] = { name: item.name, qty: 0, total: 0, costUnit: cost, totalCost: 0, isCourtesy: isCourtesy };
                      }
                      
                      productMap[key].qty += qty;
                      // El total en dinero solo suma si no es cortesía
                      if (!isCourtesy) {
          ...
          setSessionStats(prev => ({ 
              ...
              courtesyTotal: courtesyTotalVal, // Nuevo
              courtesyCost: courtesyCostVal,   // Nuevo
