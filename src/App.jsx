// src/App.jsx - CORRECCIÓN DE LOGIN Y REUBICACIÓN DE BOTÓN IMPRESORA
...
export default function App() {
  ...
  const inventoryStats = useMemo(() => { let totalCost = 0; let totalRetail = 0; let totalItems = 0; items.forEach(item => { if (item.category === 'Servicios') return; const stock = parseFloat(item.stock) || 0; const cost = parseFloat(item.cost) || 0; const price = parseFloat(item.price) || 0; if (stock > 0) { totalCost += (stock * cost); totalRetail += (stock * price); totalItems += stock; } }); return { totalCost, totalRetail, totalItems, potentialProfit: totalRetail - totalCost }; }, [items]);
  ...
  const handleStaffPinLogin = async (member) => { 
      // Esta función es asíncrona, View.jsx debe esperar o no bloquear la UI
  ...
  const handleSendToKitchen = async (cart, clearCart) => { if (!checkRegisterStatus(false)) return; if (cart.length === 0) return; const toastId = toast.loading('Procesando...'); try { const total = cart.reduce((acc, item) => acc + (item.price * item.qty), 0); const orderData = { date: new Date().toISOString(), staffId: staffMember ? staffMember.id : 'anon', staffName: staffMember ? staffMember.name : 'Mesero', orderId: 'ORD-' + Math.floor(Math.random() * 10000), items: cart, total: total, status: 'pending' }; await addDoc(collection(db, isPersonalProject ? 'pending_orders' : `${ROOT_COLLECTION}pending_orders`), orderData); const preCheckData = { ...orderData, type: 'order', date: new Date().toLocaleString(), autoPrint: true }; clearCart([]); setLastSale(preCheckData); toast.success('Enviado', { id: toastId }); setView('receipt_view'); } catch (error) { toast.error('Error', { id: toastId }); } };
  const handleVoidAndPrint = async (order) => { try { await deleteDoc(doc(db, isPersonalProject ? 'pending_orders' : `${ROOT_COLLECTION}pending_orders`, order.id)); setLastSale({ ...order, type: 'void', businessName: appName, date: new Date().toLocaleString() }); toast.success("Anulado"); setView('receipt_view'); } catch (error) { toast.error("Error"); } };
  const handleReprintOrder = (order) => { setLastSale({ ...order, type: 'order', businessName: appName, date: new Date().toLocaleString() }); setView('receipt_view'); toast.success("Reimprimiendo..."); };
  const handleFinalizeSale = async (paymentResult) => { if (!db) return; if (staffMember && staffMember.role !== 'Cajero' && staffMember.role !== 'Administrador') { toast.error("Solo Cajeros pueden cobrar."); setIsPaymentModalOpen(false); return; } if (!registerSession) { toast.error("Caja cerrada"); return; } const toastId = toast.loading('Procesando...'); setIsPaymentModalOpen(false); const itemsToProcess = orderToPay ? orderToPay.items : pendingSale.cart; const { paymentsList, totalPaid, change } = paymentResult; const totalToProcess = totalPaid - change; try { const batchPromises = []; const timestamp = new Date(); let cashierName = staffMember ? staffMember.name : 'Administrador'; const waiterName = orderToPay ? (orderToPay.staffName || 'Barra') : (staffMember ? staffMember.name : 'Barra'); const cleanItems = itemsToProcess.map(item => ({ id: item.id || 'unknown', name: item.name || 'Sin nombre', price: parseFloat(item.price) || 0, qty: parseInt(item.qty) || 1, category: item.category || 'General', stock: item.stock !== undefined ? item.stock : null, image: item.image || null, isServiceItem: !!item.isServiceItem, location: item.location || null })); const saleData = { date: timestamp.toISOString(), total: parseFloat(totalToProcess) || 0, items: cleanItems, staffId: orderToPay?.staffId || staffMember?.id || 'anon', staffName: waiterName, cashier: cashierName, registerId: registerSession.id, payments: paymentsList || [], totalPaid: parseFloat(totalPaid) || 0, changeGiven: parseFloat(change) || 0 }; const docRef = await addDoc(collection(db, isPersonalProject ? 'sales' : `${ROOT_COLLECTION}sales`), saleData); cleanItems.forEach(item => { if (item.stock !== null && item.stock !== '' && !isNaN(item.stock) && !item.isServiceItem) { const newStock = parseInt(item.stock) - item.qty; batchPromises.push(updateDoc(doc(db, getCollName('items'), item.id), { stock: newStock })); } }); if (orderToPay && orderToPay.type !== 'quick_sale') { batchPromises.push(deleteDoc(doc(db, isPersonalProject ? 'pending_orders' : `${ROOT_COLLECTION}pending_orders`, orderToPay.id))); } await Promise.all(batchPromises); const receiptData = { type: 'order', businessName: appName, date: timestamp.toLocaleString(), staffName: waiterName, cashierName: cashierName, orderId: docRef.id, items: cleanItems, total: totalToProcess, payments: paymentsList, change: change, autoPrint: true }; setLastSale(receiptData); if (pendingSale && pendingSale.clearCart) pendingSale.clearCart([]); setPendingSale(null); setOrderToPay(null); toast.success('Cobrado', { id: toastId }); setView('receipt_view'); } catch (e) { console.error(e); toast.error('Error', { id: toastId }); } };
  ...
  const handleAddStaff = async (d) => { await addDoc(collection(db, getCollName('staff')), d); toast.success('Creado'); };
  const handleUpdateStaff = async (id, d) => { await updateDoc(doc(db, getCollName('staff'), id), d); toast.success('Actualizado'); };
          ...
          <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            {isAdminMode && (
              <>
                ...
                {view === 'cashier' && (<CashierView items={items} categories={categories} tables={tables} onProcessPayment={handleStartPaymentFromCashier} onVoidOrder={handleVoidAndPrint} onReprintOrder={handleReprintOrder} onStopService={handleStopService} onOpenExpense={() => setIsExpenseModalOpen(true)} onPrintReceipt={handleBulkReceipt} />)}
