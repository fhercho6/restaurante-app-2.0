// src/App.jsx - VERSIÓN FINAL: DASHBOARD DE VALOR DE INVENTARIO + GESTIÓN COMPLETA
...
export default function App() {
  ...
  // --- CÁLCULO DE VALOR DE INVENTARIO (MEMOIZED) ---
  const inventoryStats = useMemo(() => {
      ...
      items.forEach(item => {
          // Ignorar servicios (no tienen stock físico)
  ...
  const handleAddExpense = async (description, amount) => { 
      ...
      try { 
          const expenseData = { registerId: registerSession.id, description, amount, date: new Date().toISOString(), createdBy: staffMember ? staffMember.name : 'Admin' }; 
          const colName = isPersonalProject ? 'expenses' : `${ROOT_COLLECTION}expenses`; 
          await addDoc(collection(db, colName), expenseData); 
          const expenseReceipt = { type: 'expense', businessName: appName, date: new Date().toLocaleString(), staffName: staffMember ? staffMember.name : 'Admin', description: description, amount: amount };
          ...
          <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            {isAdminMode && (
              <>
                ...
                {/* --- SECCIÓN DE ADMIN (CON DASHBOARD DE VALOR) --- */}
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        <div className="bg-white p-4 rounded-xl border border-blue-100 shadow-sm flex items-center justify-between">
                            <div><p className="text-xs text-gray-500 uppercase font-bold">Inversión (Costo)</p><p className="text-2xl font-black text-blue-600">Bs. {inventoryStats.totalCost.toFixed(2)}</p></div>
                        ...
                        <div className="bg-white p-4 rounded-xl border border-green-100 shadow-sm flex items-center justify-between">
                            <div><p className="text-xs text-gray-500 uppercase font-bold">Venta Potencial</p><p className="text-2xl font-black text-green-600">Bs. {inventoryStats.totalRetail.toFixed(2)}</p></div>
                            <div className="p-3 bg-green-50 rounded-full text-green-600"><TrendingUp size={24}/></div>
                        </div>
                        <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between">
                            <div><p className="text-xs text-gray-500 uppercase font-bold">Total Unidades</p><p className="text-2xl font-black text-gray-800">{inventoryStats.totalItems}</p></div>
                            <div className="p-3 bg-gray-100 rounded-full text-gray-600"><Package size={24}/></div>
                        </div>
                    </div>
