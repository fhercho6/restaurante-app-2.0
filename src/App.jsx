...
export default function App() {
  ...
  const inventoryStats = useMemo(() => {
      ...
      items.forEach(item => {
          ...
          if (stock > 0) { totalCost += (stock * cost); totalRetail += (stock * price); totalItems += stock; }
          if (stock > 0 && stock <= 5) lowStockCount++;
      });
      return { totalCost, totalRetail, totalItems, lowStockCount, potentialProfit: totalRetail - totalCost };
  }, [items]);
  ...
  const handleStaffPinLogin = async (member) => { 
      ...
      try { await updateDoc(doc(db, getCollName('staff'), member.id), { activeSessionId: newSessionId }); const memberWithSession = { ...member, activeSessionId: newSessionId }; setStaffMember(memberWithSession); if (member.role === 'Cajero' || member.role === 'Administrador') { setView('cashier'); toast.success(`Caja abierta: ${member.name}`); } else { setView('pos'); toast.success(`Turno iniciado: ${member.name}`); } } catch (error) { toast.error("Error de conexión"); } 
  };
  
  const handleClockAction = async (member, type) => {
      try {
          const timestamp = new Date();
          const logData = { staffId: member.id, staffName: member.name, type: type, timestamp: timestamp.toISOString(), dateStr: timestamp.toLocaleDateString() };
          const colName = isPersonalProject ? 'attendance_logs' : `${ROOT_COLLECTION}attendance_logs`;
          await addDoc(collection(db, colName), logData);
          const timeStr = timestamp.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
          const message = type === 'entry' ? `👋 ¡Hola ${member.name}! Entrada: ${timeStr}` : `🏠 ¡Hasta luego ${member.name}! Salida: ${timeStr}`;
          toast.success(message, { duration: 4000, icon: type === 'entry' ? '⏰' : '🏁' });
          if (type === 'exit') setView('landing');
