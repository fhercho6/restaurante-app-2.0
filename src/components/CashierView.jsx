// src/components/CashierView.jsx - VERSIÓN FINAL (Bloqueo de Cobro en Servicios Activos + Colores)
...
const CashierView = ({ onProcessPayment, onVoidOrder, onReprintOrder, onStopService }) => {
  ...
  const handleVoidClick = (order) => {
    // Si es un ticket de servicio activo, permitimos borrarlo (para limpiar pantalla) pero advertimos
  ...
  return (
    <div className="animate-in fade-in pb-20">
      ...
      {/* --- SECCIÓN SERVICIOS ACTIVOS (Relojes) --- */}
      ...
      {orders.length === 0 && activeServices.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {orders.map(order => {
                // DETECTAR SI ES TICKET DE SERVICIO EN CURSO (Start Ticket)
                const isServiceStart = order.items.some(i => i.name.includes('⏱️ INICIO'));
                
                // Color Code: Morado si es servicio, Blanco si es comida normal
                ...
                return (
                    <div key={order.id} className={`${cardStyle} rounded-xl shadow-sm hover:shadow-md transition-all overflow-hidden flex flex-col relative`}>
                        ...
                        {/* Etiqueta Visual de Vinculación */}
                        {isServiceStart && (
                            <div className="absolute top-0 right-0 bg-purple-100 text-purple-700 text-[9px] font-bold px-2 py-1 rounded-bl-lg uppercase tracking-wider">
                                ⏳ Servicio Activo
                            </div>
                        )}
                        ...
                        <div className="p-3 border-t bg-white grid grid-cols-5 gap-2">
                            ...
                            {/* BOTÓN BORRAR (Siempre disponible para limpiar) */}
                            ...
                            {/* BOTÓN COBRAR (LÓGICA BLINDADA) */}
                            {isServiceStart ? (
                                // SI ES SERVICIO ACTIVO -> BOTÓN BLOQUEADO GRIS
                                <button disabled className="col-span-1 bg-gray-100 text-gray-400 rounded-lg flex flex-col items-center justify-center cursor-not-allowed border border-gray-200">
                                    <Hourglass size={14}/>
                                    <span className="text-[8px] font-bold uppercase leading-none mt-0.5">En Curso</span>
                                </button>
                            ) : (
                                // SI ES COMIDA NORMAL -> BOTÓN VERDE NORMAL
