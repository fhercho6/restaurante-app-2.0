// src/components/CashierView.jsx - CORREGIDO (Clics, Imágenes y Categorías)
import React, { useState, useEffect } from 'react';
import { collection, query, orderBy, onSnapshot } from 'firebase/firestore';
import { db, ROOT_COLLECTION, isPersonalProject } from '../config/firebase';
import { 
  Clock, CheckCircle, XCircle, Printer, Coffee, DollarSign, 
  Search, Grid, List, ShoppingCart, Trash2, ChevronRight, Plus, Minus, Tag
} from 'lucide-react';

// --- SUB-COMPONENTE: TARJETA DE PRODUCTO (MEJORADA) ---
const CashierProductCard = ({ item, onClick }) => {
    ...
    return (
      ...
      >
        {/* IMAGEN: Usamos object-contain para que la botella entre completa */}
...
export default function CashierView({ items, categories, onProcessPayment, onVoidOrder, onReprintOrder, onStopService, onOpenExpense }) {
  ...
  const addToCart = (item) => {
    // Vibración en móviles si es soportado
  ...
  // Filtros de Productos (Optimizados)
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-100px)] animate-in fade-in">
      ...
      <div className="flex justify-between items-center mb-3 px-1 flex-wrap gap-2">
         <div className="flex bg-white p-1 rounded-xl shadow-sm border border-gray-200">
             <button 
                ...
                className={`px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-all ${activeTab === 'orders' ? 'bg-orange-500 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}
             >
             ...
             <button 
                ...
                className={`px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-all ${activeTab === 'quick' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}
             >
      ...
      <div className="flex-1 bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden relative">
            ...
            <div className="absolute inset-0 overflow-y-auto p-4 bg-gray-50/50">
                {orders.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-gray-300">
                        ...
                        <p className="font-bold text-lg">Sin comandas pendientes</p>
                    </div>
                ) : (
                                ...
                                <div className={`p-3 flex justify-between items-start ${order.status === 'pending' ? 'bg-orange-50' : 'bg-gray-100'}`}>
                                    <div>
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            <span className="text-[10px] font-bold bg-white px-2 py-0.5 rounded text-gray-500 border border-gray-200">{new Date(order.date).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                                        </div>
                                    ...
                                    <div className="text-right">
                                        <p className="font-black text-lg text-gray-800">Bs. {order.total?.toFixed(2)}</p>
                                    </div>
                                </div>
                                <div className="p-3 flex-1 overflow-y-auto max-h-32 space-y-1">
                                    {order.items?.map((item, idx) => (
                                        <div key={idx} className="flex justify-between text-xs items-center border-b border-gray-50 pb-1 last:border-0">
                                            <div className="flex gap-2 items-center">
                                                <span className="font-bold text-gray-800 w-5 text-center bg-gray-100 rounded text-[10px]">{item.qty}</span>
         ...
         {/* --- PESTAÑA 2: VENTA RÁPIDA (MEJORADA) --- */}
         {activeTab === 'quick' && (
             <div className="absolute inset-0 flex">
                     ...
                     <div className="p-3 bg-white border-b border-gray-200 space-y-3 z-10 shadow-sm">
                         <div className="relative">
                             ...
                             <input 
                                ...
                                className="w-full pl-9 p-2 bg-gray-100 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500 transition-all" 
                                value={searchTerm} 
                                onChange={e => setSearchTerm(e.target.value)} 
                             />
                         </div>
                         
                         {/* Categorías mejoradas: Grid compacto para ver más */}
                         <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                             <button 
                                onClick={() => setCatFilter('Todos')} 
                                className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap border transition-colors ${catFilter === 'Todos' ? 'bg-black text-white border-black' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'}`}
                             >
                                Todos
                             </button>
                             {categories?.filter(c => c !== 'Servicios').map(cat => (
                                 <button 
                                    ...
                                    className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap border transition-colors ${catFilter === cat ? 'bg-black text-white border-black' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'}`}
                                 >
                                    {cat}
                                 </button>
                             ))}
                 ...
                 <div className="w-80 bg-white flex flex-col shadow-2xl z-20 border-l border-gray-200">
                     <div className="p-3 bg-blue-50 border-b border-blue-100 flex justify-between items-center">
                         <h3 className="font-black text-blue-900 flex items-center gap-2 text-sm"><ShoppingCart size={16}/> Venta Directa</h3>
                         {cart.length > 0 && <button onClick={() => setCart([])} className="text-[10px] text-red-500 font-bold hover:underline bg-white px-2 py-1 rounded border border-red-100">LIMPIAR</button>}
                     </div>
                     ...
                     <div className="flex-1 overflow-y-auto p-3 space-y-2">
                         {cart.length === 0 ? (
                             <div className="h-full flex flex-col items-center justify-center text-gray-300 space-y-2 select-none">
                                 ...
                                 <p className="text-xs font-medium">Escanea o selecciona</p>
                             </div>
                         ) : (
                             cart.map(item => (
                                 <div key={item.id} className="flex justify-between items-center bg-white border border-gray-100 p-2 rounded-lg shadow-sm animate-in slide-in-from-right duration-200">
                                     <div className="flex-1 min-w-0 mr-2">
                                         ...
                                         <div className="text-[10px] text-gray-500">unitario: Bs. {item.price}</div>
                                     </div>
