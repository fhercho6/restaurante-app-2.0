...
import { ChefHat, User, Settings, UtensilsCrossed, PartyPopper, Sparkles, Clock, Wine, Star, Calculator, Music, Crown } from 'lucide-react';
  ...
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden z-10">
      {particles.map((_, i) => {
        ...
        const color = ['bg-yellow-400', 'bg-purple-500', 'bg-green-500', 'bg-red-500', 'bg-blue-400'][Math.floor(Math.random() * 5)];
        return (
          <div
            key={i}
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden font-sans bg-[#050505]">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden">
        <div className="absolute top-[-20%] left-[-20%] w-[50%] h-[50%] bg-purple-600/30 blur-[120px] animate-pulse"></div>
        <div className="absolute bottom-[-20%] right-[-20%] w-[50%] h-[50%] bg-green-600/30 blur-[120px] animate-pulse delay-1000"></div>
        <div className="absolute top-[20%] right-[10%] w-[20%] h-[20%] bg-yellow-600/30 blur-[100px] animate-pulse delay-500"></div>
      </div>
      <Confetti />
      ...
      <div className="absolute top-0 inset-x-0 h-16 z-20 flex items-center justify-between px-6 bg-gradient-to-b from-black/90 to-transparent border-b border-yellow-500/30 shadow-[0_5px_15px_-5px_rgba(234,179,8,0.2)]">
        ...
        <div className="flex gap-2 text-purple-400 drop-shadow-[0_0_8px_rgba(192,132,252,0.8)]">
          <Music size={20} className="animate-pulse" /><Crown size={16} className="mt-1 animate-bounce-slow" />
        </div>
      ...
      <div className="relative z-30 flex flex-col items-center w-full max-w-md px-6 text-center animate-in fade-in zoom-in duration-700 mt-16">
        ...
        <div className="mb-6 relative group">
          <div className="absolute inset-0 bg-gradient-to-tr from-purple-500 to-green-500 blur-3xl opacity-30 group-hover:opacity-50 transition-opacity duration-1000 rounded-full"></div>
          <div className="relative w-32 h-32 bg-black/80 backdrop-blur-md border border-purple-500/50 rounded-full flex items-center justify-center shadow-[0_0_40px_-5px_rgba(168,85,247,0.4)] p-4 ring-1 ring-purple-400/30">
            {logo ? (
              <img src={logo} alt="Logo" className="w-full h-full object-contain drop-shadow-lg" />
            ) : (
              <Crown size={64} className="text-purple-200 drop-shadow-[0_0_15px_rgba(168,85,247,0.5)]" strokeWidth={1.5} />
            )}
          </div>
        </div>
        ...
        <div className="flex items-center justify-center gap-2 mb-2">
          <PartyPopper size={28} className="text-purple-500 drop-shadow-[0_0_10px_rgba(168,85,247,0.8)] -rotate-12" />
          <h1 className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-yellow-200 to-green-400 tracking-tight drop-shadow-[0_0_15px_rgba(168,85,247,0.3)] uppercase relative">
            {appName || 'LicoBar'}
          </h1>
          <Music size={28} className="text-green-500 drop-shadow-[0_0_10px_rgba(34,197,94,0.8)] rotate-12" />
        </div>
        <p className="text-sm text-yellow-500/90 font-bold tracking-[0.3em] uppercase mb-10 flex items-center gap-2 before:h-[1px] before:w-6 before:bg-green-500 after:h-[1px] after:w-6 after:bg-green-500">
          Carnaval 2026
        </p>
      ...
      <div className="absolute bottom-6 text-white/20 text-[10px] uppercase font-bold tracking-[0.3em] z-30 flex items-center gap-2">
        <Crown size={10} className="text-purple-600" /> CARNAVAL EDITION <Crown size={10} className="text-purple-600" />
      </div>
