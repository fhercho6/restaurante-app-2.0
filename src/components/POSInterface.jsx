// src/components/POSInterface.jsx - VERSIÓN BLINDADA (Tarjeta Integrada + Fix Altura)
import React, { useState } from 'react';
// Usamos SOLO iconos seguros
...
export default function POSInterface({ items, categories, staffMember, onCheckout, onPrintOrder, onExit, onOpenServiceModal }) {
  ...
  // --- COMPONENTE INTERNO DE TARJETA (Para evitar errores de importación) ---
  const POSCard = ({ item, onClick }) => (
    <div onClick={onClick} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-all cursor-pointer flex flex-col h-40 relative group">
      {/* Imagen / Fondo */}
  ...
  return (
    // FIX ALTURA: Usamos h-[85vh] para que no se salga de la pantalla del App.jsx
