// src/components/POSInterface.jsx - VERSIÓN FINAL (Con Servicios por Hora)
import React, { useState } from 'react';
import { Search, ShoppingCart, Trash2, LogOut, ChevronLeft, Send, Clock } from 'lucide-react';
...
export default function POSInterface({ items, categories, staffMember, onCheckout, onPrintOrder, onExit, onOpenServiceModal }) {
  ...
  // Filtramos para que NO aparezcan los servicios de hora en el menú normal de productos
  // (Para evitar que los agreguen como productos normales por error)
  const filteredItems = items.filter(i => {
    const matchesCat = categoryFilter === 'Todos' ? true : i.category === categoryFilter;
    const matchesSearch = i.name.toLowerCase().includes(searchTerm.toLowerCase());
    const isNotService = i.category !== 'Servicios'; // Ocultamos la categoría Servicios del menú normal
    ...
    <div className="flex h-screen bg-gray-100 overflow-hidden animate-in fade-in">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="bg-white p-4 shadow-sm z-10">
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-3">
               ...
               <div>
                 ...
                 <span className="text-xs text-green-600 font-bold uppercase">Turno Activo</span>
               </div>
            </div>
            
            {/* BOTÓN NUEVO: SERVICIO POR HORA */}
            <button 
              onClick={onOpenServiceModal}
              className="bg-purple-600 text-white px-4 py-2 rounded-full font-bold shadow-md hover:bg-purple-700 transition-all flex items-center gap-2 animate-pulse"
            >
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 hide-scrollbar">
            <button onClick={() => setCategoryFilter('Todos')} className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-colors ${categoryFilter === 'Todos' ? 'bg-black text-white' : 'bg-gray-100 text-gray-600'}`}>Todos</button>
            {categories.filter(c => c !== 'Servicios').map(cat => (
              <button key={cat} onClick={() => setCategoryFilter(cat)} className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-colors ${categoryFilter === cat ? 'bg-black text-white' : 'bg-gray-100 text-gray-600'}`}>{cat}</button>
            ))}
          </div>
      ...
      <div className="w-96 bg-white shadow-2xl flex flex-col border-l border-gray-200 z-20">
        <div className="p-4 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
          <h3 className="font-bold text-gray-700 flex items-center gap-2"><ShoppingCart size={20}/> Comanda Actual</h3>
        ...
        <div className="p-4 bg-gray-50 border-t border-gray-200 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
             <button onClick={() => onPrintOrder(cart, setCart)} disabled={cart.length === 0} className="bg-gray-800 hover:bg-gray-900 text-white py-4 rounded-xl font-bold shadow-lg flex flex-col items-center justify-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed transition-all">
                <Send size={20} /> <span className="text-xs uppercase">Enviar a Caja</span>
             </button>
             <button onClick={() => onCheckout(cart, setCart)} disabled={cart.length === 0} className="bg-green-600 hover:bg-green-700 text-white py-4 rounded-xl font-bold shadow-lg flex flex-col items-center justify-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed transition-all">
