...
export default function StaffManagerView({ staff, roles, onAddStaff, onUpdateStaff, onDeleteStaff, onManageRoles, onPrintCredential }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (isEditing && currentMember) {
      onUpdateStaff(currentMember.id, formData);
    } else {
      onAddStaff(formData);
    }
    resetForm();
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 mb-8 no-print">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col md:flex-row gap-4 items-end">
          <div className="flex-1 w-full">
            ...
            <input type="text" placeholder="Ej. Juan Pérez" className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} required />
          </div>
          ...
          <div className="w-full md:w-32">
            ...
            <input type="text" maxLength="4" placeholder="1234" className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none font-mono text-center" value={formData.pin} onChange={e => setFormData({...formData, pin: e.target.value.replace(/\D/g,'')})} required />
          </div>
          <div className="flex gap-2 w-full md:w-auto">
             <button type="submit" className={`px-6 py-2 rounded-lg font-bold text-white shadow-md transition-all whitespace-nowrap flex-1 ${isEditing ? 'bg-blue-600 hover:bg-blue-700' : 'bg-blue-600 hover:bg-blue-700'}`}>{isEditing ? 'Actualizar' : 'Registrar'}</button>
             {isEditing && <button type="button" onClick={resetForm} className="px-4 py-2 rounded-lg font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 transition-all">Cancelar</button>}
          </div>
        </form>
      </div>
              ...
              <tr key={member.id} className="hover:bg-gray-50 transition-colors group">
                ...
                <td className="p-4 text-right">
                  <div className="flex justify-end gap-2 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button 
                        onClick={() => onPrintCredential(member)} 
