...
export default function StaffManagerView({
  ...
  const resetForm = () => {
    setFormData({ name: '', role: 'Garzón', pin: '', dailySalary: '' });
    setIsEditing(false);
    setCurrentMember(null);
  };

  const handleEditClick = (member) => {
    setFormData({ name: member.name, role: member.role, pin: member.pin, dailySalary: member.dailySalary || '' });
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 mb-8 no-print">
        ...
        <form onSubmit={handleSubmitSafe} className="flex flex-col md:flex-row gap-4 items-end">
          ...
          <div className="w-full md:w-32">
            <label className="text-xs font-bold text-gray-500 uppercase mb-1 block">Sueldo Diario</label>
            <div className="relative">
              <span className="absolute left-2 top-2 text-gray-400 font-bold text-xs">Bs.</span>
              <input type="number" placeholder="0" className="w-full p-2 pl-6 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" value={formData.dailySalary} onChange={e => setFormData({ ...formData, dailySalary: e.target.value })} disabled={isSaving} />
            </div>
          </div>
        ...
        <table className="w-full text-left">
          <thead className="bg-gray-50 text-xs uppercase text-gray-500 font-bold">
            <tr>
              ...
              <th className="p-4">Sueldo Diario</th>
