// src/components/Receipt.jsx - CON NOMBRE DE CAJERO
...
const Receipt = ({ data, onPrint, onClose, printerType = 'thermal' }) => {
  ...
  const renderThermalReport = () => {
      ...
      if (data.soldProducts) {
          zReportRows = data.soldProducts.map(p => `<div class="row"><div class="col-qty">${p.qty}</div><div class="col-name">${p.name}</div><div class="col-price">${fmt(p.total)}</div></div>`).join('');
      }
        ...
        <body>
            <div class="text-center border-b">
                ...
                <div style="font-size:10px; margin-top:4px; display:flex; justify-content:space-between;">
                    <span>Atiende: ${staffName.split(' ')[0]}</span>
                    <span>Cajero: ${cashierName.split(' ')[0]}</span>
                </div>

                ${data.orderId ? `<div style="font-size:10px; margin-top:2px;">Folio: #${data.orderId.slice(-6)}</div>` : ''}
            </div>
            ...
            ` : `
                ...
                <div class="border-b" style="margin:5px 0;"></div>
                ${isCourtesySale ? `
                    <div class="courtesy-box"><div class="bold" style="font-size:14px;">CORTESÍA AUTORIZADA</div><div style="font-size:10px;">Total Bonificado: Bs. ${fmt(data.total)}</div></div>
    ...
    <div className="fixed inset-0 bg-gray-900/90 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in">
      <div className="bg-white w-full max-w-sm shadow-2xl rounded-xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="bg-gray-800 p-3 flex justify-between items-center">
          <h3 className="text-white font-bold text-sm">{useThermalFormat ? 'TICKET DE VENTA' : 'REPORTE CARTA'}</h3>
        ...
        <div className="p-6 bg-gray-100 flex justify-center">
            <div className={`bg-white p-4 shadow text-center w-full border-t-4 ${isCourtesySale ? 'border-yellow-500' : 'border-blue-500'}`}>
                ...
                <p className="text-xs text-gray-400 mt-4 uppercase font-bold border-t pt-2">Formato: {useThermalFormat ? 'TÉRMICO (80mm)' : 'CARTA (A4)'}</p>
            </div>
        </div>
      </div>
    </div>
