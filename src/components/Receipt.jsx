// src/components/Receipt.jsx - CORRECCIÓN: COLUMNAS SEPARADAS (VENTA/CORTESÍA)
...
const Receipt = ({ data, onPrint, onClose, printerType = 'thermal' }) => {
  ...
  // --- MODO 1: REPORTE CARTA ---
  const renderLetterReport = () => {
      const stats = data.stats || {};
      
      // AQUÍ ESTÁ LA MAGIA: Usamos qtySold y qtyCourtesy
      const productRows = data.soldProducts ? data.soldProducts.map(p => `
        <tr>
            ...
            <td style="text-align:center;padding:6px;border-bottom:1px solid #eee;">${p.qtySold || 0}</td>
            <td style="text-align:center;padding:6px;border-bottom:1px solid #eee;">${p.qtyCourtesy || 0}</td>
  ...
  const renderThermalReport = () => {
      ...
      // En térmico solo mostramos totales para ahorrar papel, o una versión simplificada
