// src/components/Receipt.jsx - RECIBO QUE RECONOCE CORTESÍA
...
const Receipt = ({ data, onPrint, onClose, printerType = 'thermal' }) => {
  ...
  // DETECTAR SI ES CORTESÍA (Busca en la lista de pagos)
  const isCourtesySale = data.payments && data.payments.some(p => p.method === 'Cortesía');

  // TÍTULO DINÁMICO
  let displayTitle = data.businessName || 'LicoBar';
  if (data.type === 'expense') displayTitle = 'VALE DE GASTO';
  if (isCourtesySale) displayTitle = 'RECIBO DE CORTESÍA'; // <--- TÍTULO ESPECIAL
  ...
  const renderLetterReport = () => {
      ...
      return `<html><head><title>Reporte</title><style>body{font-family:'Arial',sans-serif;font-size:11px;margin:30px;color:#000}.header-section{text-align:center;margin-bottom:20px}.report-title{font-size:18px;font-weight:bold;text-decoration:underline;margin-bottom:5px;text-transform:uppercase}.report-meta{font-size:12px;margin-bottom:20px}table{width:100%;border-collapse:collapse;margin-bottom:30px;font-size:11px}th{border-top:2px solid #000;border-bottom:2px solid #000;padding:6px;font-weight:bold;text-transform:uppercase;background-color:#f9f9f9}td{padding:6px;vertical-align:top}.financial-table td{border-bottom:1px dotted #ccc}.financial-table th{text-align:left}.product-table th.left{text-align:left}.product-table th.center{text-align:center}.product-table th.right{text-align:right}.product-table td{font-family:'Courier New',monospace;font-size:12px}.footer-total{font-size:14px;font-weight:bold;text-align:right;border-top:2px solid #000;padding-top:5px;margin-top:-20px}.text-right{text-align:right}.box{flex:1;border:1px solid #000;padding:15px}.main-container{display:flex;gap:40px;margin-bottom:30px}.clean-table{width:100%;border-collapse:collapse}.clean-table td{padding:5px 0;border-bottom:1px dashed #ddd}.section-header{background-color:#000;color:#fff;padding:5px 10px;font-weight:bold;margin-top:20px}</style></head><body>
            ...
            ` : `<div style="border:1px solid #000;padding:20px;"><h2 style="text-align:center;">DETALLE VENTA</h2><table class="product-table"><thead><tr><th>CANT</th><th>DESCRIPCION</th><th class="right">TOTAL</th></tr></thead><tbody>${data.items ? data.items.map(i => `<tr><td style="text-align:center">${i.qty}</td><td>${i.name}</td><td class="right">${fmt(i.price * i.qty)}</td></tr>`).join('') : ''}</tbody></table><h1 class="right" style="margin-top:20px;">TOTAL: Bs. ${fmt(data.total)}</h1></div>`}
      </body></html>`;
  };

  // --- MODO TÉRMICO (TICKET) ---
  const renderThermalReport = () => {
      // Si es cortesía, los precios salen en 0.00 en la columna total para que no confunda
      let itemsHtml = data.items ? data.items.map(item => `
        ...
        <head>
            <style>
                * { box-sizing: border-box; }
                ...
                .courtesy-banner { border: 2px dashed #000; padding: 5px; margin: 10px 0; text-align: center; font-weight: bold; font-size: 14px; }
            </style>
        </head>
        <body>
            <div class="text-center border-b">
                <div class="bold" style="font-size:16px;">${data.type==='z-report'?'CIERRE DE CAJA': displayTitle}</div>
                <div>${data.date}</div>
                ${isCourtesySale ? '<div class="bold" style="font-size:10px;">(NO TIENE VALOR COMERCIAL)</div>' : ''}
            </div>

            ${data.type === 'expense' ? `
                <div style="margin-top:10px;"><span style="font-size:10px;">CONCEPTO:</span><br/><span class="bold uppercase" style="font-size:14px;">${data.description}</span><div class="border-b" style="margin:5px 0;"></div><div class="flex-between bold" style="font-size:18px;"><span>MONTO:</span><span>Bs. ${fmt(data.amount)}</span></div><br/><br/><div class="text-center" style="font-size:10px;border-top:1px solid #000;padding-top:5px;">FIRMA</div></div>
            ` : data.type === 'z-report' ? `
                <div class="flex-between"><span>Fondo Inicial:</span><span>${fmt(data.openingAmount)}</span></div><div class="flex-between bold"><span>(+) Ventas:</span><span>${fmt((stats.cashSales||0)+(stats.digitalSales||0))}</span></div><div style="font-size:10px;margin-left:10px;"><div class="flex-between"><span>Efvo: ${fmt(stats.cashSales)}</span><span>Dig: ${fmt(stats.digitalSales)}</span></div></div><div class="flex-between"><span>(-) Gastos:</span><span>${fmt(stats.totalExpenses)}</span></div><div class="border-b" style="margin:5px 0;"></div><div class="flex-between bold" style="font-size:18px;"><span>CAJA:</span><span>Bs. ${fmt(data.finalCash)}</span></div><br/><div>DETALLE PRODUCTOS</div>${zReportRows}
            ` : `
                ${itemsHtml}
                ...
                ${isCourtesySale ? `
                    <div class="courtesy-banner">CORTESÍA / REGALO</div>
                    <div class="flex-between bold"><span>TOTAL A PAGAR:</span><span>Bs. 0.00</span></div>
                    <div style="text-align:right; font-size:10px; text-decoration:line-through;">Valor Real: Bs. ${fmt(data.total)}</div>
                ` : `
                    <div class="flex-between bold" style="font-size:18px;"><span>TOTAL:</span><span>Bs. ${fmt(data.total)}</span></div>
                    ${data.payments ? `<div style="margin-top:5px; font-size:10px;">${data.payments.map(p=>`<div>PAGO ${p.method.toUpperCase()}: ${fmt(p.amount)}</div>`).join('')}</div>` : ''}
                    ${data.changeGiven > 0 ? `<div class="text-right bold">CAMBIO: ${fmt(data.changeGiven)}</div>`:''}
                `}
                
            `}
            <div style="margin-top:15px;text-align:center;font-size:10px;">*** GRACIAS ***</div>
        </body>
        </html>
      `;
  };
      ...
      <div className="bg-white w-full max-w-sm shadow-2xl rounded-xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 bg-gray-100 flex justify-center">
            <div className={`bg-white p-4 shadow text-center w-full border-t-4 ${isCourtesySale ? 'border-yellow-500' : 'border-blue-500'}`}>
                <p className="font-bold text-lg mb-2">{isCourtesySale ? 'RECIBO DE CORTESÍA' : (data.businessName || 'LicoBar')}</p>
                <p className="text-3xl font-black text-gray-800">Bs. {isCourtesySale ? '0.00' : fmt(previewAmount)}</p>
                {isCourtesySale && <p className="text-xs text-red-500 font-bold mt-2">NO VALIDO COMO FACTURA</p>}
            </div>
        </div>
      </div>
    </div>
